...
final class ShellBridge: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "notchline.shell-bridge")
...
enum ShellIntegration {
    static let scriptURL = PrefsStore.directory
        ...
        .appendingPathComponent("notchline.zsh")

    static var rcURL: URL {
        URL(fileURLWithPath: Paths.home).appendingPathComponent(".zshrc")
    }

    private static let marker = "# Notchline: long-running commands in the notch"

    /// The line added to `~/.zshrc`. It survives the app being deleted.
    static var sourceLine: String {
        let path = "$HOME/Library/Application Support/Notchline/shell/notchline.zsh"
    ...
    static var isInstalled: Bool {
        ...
        return rc.contains("Notchline/shell/notchline.zsh")
    }
    ...
    static func install() throws {
        ...
        // one blank line before our block, however the file ends
        let existing = (try? String(contentsOf: rcURL, encoding: .utf8)) ?? ""
        let gap = existing.isEmpty || existing.hasSuffix("\n\n") ? "" : (existing.hasSuffix("\n") ? "\n" : "\n\n")
        let block = gap + "\(marker)\n\(sourceLine)\n"
        let url = rcURL
    ...
    static func uninstall() throws {
        ...
        let kept = rc.components(separatedBy: "\n").filter { !Migration.isOurRCLine($0) }
        try kept.joined(separator: "\n").write(to: url, atomically: true, encoding: .utf8)
    }

    /// Plain zsh: `zsh/net/socket` connects in-process, so a hook costs no fork.
    static let zshScript = #"""
    # Notchline shell integration for zsh — generated by the app, rewritten on launch.
    # Reports each command to Notchline so long ones show up in the notch.

    [[ -o interactive ]] || return 0
    (( ${+__notchline_loaded} )) && return 0
    typeset -g __notchline_loaded=1
    zmodload zsh/net/socket 2>/dev/null || return 0

    typeset -g __nt_sock="${NOTCHLINE_SOCKET:-$HOME/Library/Application Support/Notchline/notch.sock}"

    # `notch` for scripts started from this shell
    typeset -g __nt_bin="$HOME/Library/Application Support/Notchline/bin"
